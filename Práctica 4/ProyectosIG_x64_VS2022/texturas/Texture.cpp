...
void Texture::bind(GLuint mixMode) // GL_REPLACE, GL_MODULATE, GL_ADD
{
	...
	//[glTexEnvi] -> establecer parametros de textura relacionados con el entorno de la textura,
	//				 afecta el estado global de OpenGL y no se almacena en el objeto de textura en si.
...
void Texture::load(const std::string & BMP_Name, GLubyte alpha)
{
	...
	PixMap32RGBA pixMap;	// Variable local para cargar la imagen del archivo.
							// Representacion de una imagen donde cada pixel
							// esta compuesto por 32 bits divididos en cuatro canales: Rojo, Verde, Azul y Alfa.
  
    pixMap.load_bmp24BGR(BMP_Name); // carga y pone alpha=255
	...
	glTexImage2D(GL_TEXTURE_2D,		// puede ser tambien 1D o 3D
		...
		mWidth, mHeight,	// tam de la img (data), tienen que ser potencias de 2 (no necesariamente)
...
void Texture::loadColorBuffer(GLsizei width, GLsizei height, GLuint buffer)
{
	if (buffer == GL_FRONT || buffer == GL_FRONT) //si el buffer es valido
	{
		...
		// pixels - > array donde guardar los datos (de tipo y tam adecuado )
